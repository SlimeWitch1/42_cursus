...
static void	render_map(t_game *game, int row, int column)
{
	if (game->map->matrix[row][column] == 1)
	{
		mlx_put_image_to_window(game->mlx, game->window,
			game->wall_img, column * game->tile_width, row * game->tile_height);
	}
	else if (game->map->matrix[row][column] == C)
	{
		mlx_put_image_to_window(game->mlx, game->window,
			game->collect_img, column * game->coll_width, row * game->coll_height);
	}
	else if (game->map->matrix[row][column] == E)
	{
		...
		{
			mlx_put_image_to_window(game->mlx, game->window,
				game->exit_open_img, column * game->tile_width, row * game->tile_height);
		}
		else
		{
			mlx_put_image_to_window(game->mlx, game->window,
				game->exit_closed_img, column * game->tile_width, row * game->tile_height);
		}
	}
}
...
void	main_display(t_game *game)
{
	...
	while (row < game->map->row_and_column[0])
	{
		...
		{
			mlx_put_image_to_window(game->mlx, game->window,
				game->floor_img, column * game->tile_width, row * game->tile_height);
			if (game->map->matrix[row][column] == 1
